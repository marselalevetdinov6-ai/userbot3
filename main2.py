...
class TelegramAutoJoinBot:
    ...
    async def process_message(self, text: str, source: str):
        ...
        # Формируем отчет
        report = f"📊 **Отчет обработки от {source}**\n\n"
        report += f"📨 Сообщение: {text[:100]}...\n\n"
        report += "🔗 **Результаты:**\n"
        report += "\n".join(results)
        report += f"\n\n📈 Всего обработано: {len(links)}"
        report += f"\n✅ Успешно: {len([r for r in results if '✅' in r])}"
        report += f"\n❌ Ошибок: {len([r for r in results if '❌' in r])}"
    ...
    async def run(self):
        ...
        async def message_handler(event):
            ...
            if sender and hasattr(sender, 'username') and sender.username == TARGET_BOT:
                ...
                # # Отправляем отчет в личные сообщения
                # if report:
                #     try:
                #         await self.client.send_message(me.id, report, parse_mode='Markdown')
